...
public class DeployModelCmd implements Command<Deployment>, Serializable {
    ...
    public Deployment execute(CommandContext commandContext) {
        ...
        if (Objects.isNull(model)) {
            throw new FlowableObjectNotFoundException("未找到 id 为‘" + modelId + "’的模型!", Model.class);
        }
        if (CommonUtil.isNotEmptyAfterStrip(model.getDeploymentId())) {
            throw new FlowableException("该 id 为‘" + modelId + "’的模型已经部署过了！");
        }

        if (!model.hasEditorSource()) {
            throw new FlowableObjectNotFoundException("该 id 为‘" + modelId + "’的模型没有可用的资源 ！", String.class);
        }
        // 模型的资源
        byte[] bpmnBytes = processEngineConfiguration.getModelEntityManager().findEditorSourceByModelId(modelId);
            ...
            flowElements.forEach(fe -> {
                ...
                if (Objects.nonNull(formKey) && !formKey.isBlank()) {
                    ...
                    if (Objects.isNull(formDefinition)) {
                        throw new FlowableObjectNotFoundException("该 id 为‘" + modelId + "’的模型关联的表单: ‘" + formKey + "’尚未部署！", String.class);
                    }
                }
            });
        }
